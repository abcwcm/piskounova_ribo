---
title: "Script 2 PDX CDF stalling plots"
...
coverage_plot <- function(transcript_name,gene_name,transcript_length,sec_loc, start_pos, stop_pos){
...
write.csv(new_df,paste0("/SSD/maa7095/RIBOseq/Shino_new_analysis/inframe_downtream_analysis/Xenograft/p_site_coverage/CDF_plots/", gene_name,"_coverage_df.csv"), quote=F, row.names = F)

new_df <- new_df %>%
  mutate(counts = ifelse(str_detect(Condition, "met_liv"), -1 * counts, counts))



wt_fraq <- new_df %>% filter(Condition=="sq")
wt_fraq <- wt_fraq$fraction

ko_fraq <- new_df %>% filter(Condition=="met_liv")
ko_fraq <- ko_fraq$fraction
res_ks <- ks.test(wt_fraq, ko_fraq)
p_ks <- formatC(res_ks$p.value, format = "e", digits = 2)

pdf(file = paste0("/SSD/maa7095/RIBOseq/Shino_new_analysis/inframe_downtream_analysis/Xenograft/p_site_coverage/CDF_plots/",gene_name, "_coverage_clean_counts.pdf"),   # The directory you want to save the file in
    width = 10,
    height = 12) # 

max_index <- which.max(new_df$counts)
max_count <- new_df$counts[max_index]
ko1_name <- grep("met_liv", unique(new_df$Condition), value = T)
wt_name <- grep("sq", unique(new_df$Condition), value = T)
color_values <- c(
...
linetype_values <- c(
  ...
  pdf(file = paste0("/SSD/maa7095/RIBOseq/Shino_new_analysis/inframe_downtream_analysis/Xenograft/p_site_coverage/CDF_plots/", gene_name, "_coverage.pdf"),  
      width = 10,
      height = 12) 
...
### GPX1
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000419783.3","GPX1",899,220, 76, 685)
```


### GPX4
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000354171.13","GPX4",851,267, 51, 642)
```

### TXNRD1
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000525566.6","TXNRD1",3860,1966, 25, 1972)
```

### TXNRD2
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000400521.7","TXNRD2",1941,1582, 16, 1588)
```

### MSRB1
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000361871.8","MSRB1",1277,326, 44, 392)
```

### SELENOH
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000534355.6","SELENOH",1192,235, 106, 472)
```

### SELENOI
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000260585.12","SELENOI",8066,1249, 91, 1282)
```

### SELENOK
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000495461.6","SELENOK",1497,345, 72, 354)
```

### SELENON
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000361547.7","SELENON",4314,417, 39, 1707)
```


### SELENOO
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000380903.7","SELENOO",2283,2025, 27, 2034)
```


### SELENOP
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000514985.6","SELENOP",2056,245, 71, 1214)
```


### SELENOS
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000526049.6","SELENOS",1218,360, 69, 636)
```

### SELENOT
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000471696.6","SELENOT",3437,187, 43, 628)
```

### SELENOW
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000601048.6","SELENOW",758,120, 84, 345)
```

### SELENOF
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000331835.10","SELENOF",1542,301, 16, 511)
```

### SEPHS2
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000478753.5","SEPHS2",2244,325, 148, 1492)
```

### TXNRD3
```{r, echo=F, message=F, warning=F}
coverage_plot("ENST00000524230.9","TXNRD3",2921,2058, 135, 2064)
```
