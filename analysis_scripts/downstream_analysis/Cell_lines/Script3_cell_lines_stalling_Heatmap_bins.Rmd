---
title: "Script 3 cell lines Heatmaps for ribosome stalling"
...
heatmap_values_seleno <- function(transcript_name,gene_name,transcript_length,sec_loc, condition, cds_start, cds_stop){
  #load results from Script 1

  cover_dir <- "/SSD/maa7095/RIBOseq/Shino_new_analysis/inframe_downtream_analysis/Cell_lines/p_site_coverage"
  ...
  #Now for three prime. Calculate transcript length-sec position to know the length.
  #Now split df based on selenocystine location.
  # custom bins
...
```{r, echo=F, message=F, warning=F}
### GPX1
heatmap_df1 <- heatmap_values_seleno("ENST00000419783.3","GPX1",899,220, "no_se", 76, 687)
### GPX3

heatmap_df2 <- heatmap_values_seleno("ENST00000388825.9","GPX3",1603,281, "no_se", 65, 745)


### GPX4

heatmap_df3 <- heatmap_values_seleno("ENST00000354171.13","GPX4",851,267, "no_se", 51, 644)

### TXNRD1
heatmap_df4 <- heatmap_values_seleno("ENST00000525566.6","TXNRD1",3860,1966, "no_se", 25, 1972)

### TXNRD2
heatmap_df5 <- heatmap_values_seleno("ENST00000400521.7","TXNRD2",1941,1582, "no_se", 16, 1590)

### MSRB1
heatmap_df6 <- heatmap_values_seleno("ENST00000361871.8","MSRB1",1277,326, "no_se", 44, 394)

### SELENOH
heatmap_df7 <- heatmap_values_seleno("ENST00000534355.6","SELENOH",1192,235, "no_se", 106, 474)
### SELENOI
heatmap_df8 <- heatmap_values_seleno("ENST00000260585.12","SELENOI",8066,1249, "no_se",  91, 1284)

### SELENOK
heatmap_df9 <- heatmap_values_seleno("ENST00000495461.6","SELENOK",1497,345, "no_se", 72, 356)

### SELENON
heatmap_df10 <- heatmap_values_seleno("ENST00000361547.7","SELENON",4314,417,"no_se", 39, 1709)
### SELENOO
heatmap_df11 <- heatmap_values_seleno("ENST00000380903.7","SELENOO",2283,2025, "no_se", 27, 2036)

### SELENOS
heatmap_df12 <- heatmap_values_seleno("ENST00000526049.6","SELENOS",1218,360,"no_se", 69, 638)

### SELENOT
heatmap_df13 <- heatmap_values_seleno("ENST00000471696.6","SELENOT",3437,187,"no_se", 43, 630)

### SELENOW
heatmap_df14 <- heatmap_values_seleno("ENST00000601048.6","SELENOW",758,120, "no_se", 84, 347)

### SELENOF
heatmap_df15 <- heatmap_values_seleno("ENST00000331835.10","SELENOF",1542,301, "no_se",  16, 513)

### SEPHS2
heatmap_df16 <- heatmap_values_seleno("ENST00000478753.5","SEPHS2",2244,325, "no_se", 148, 1494)

### TXNRD3
heatmap_df17 <- heatmap_values_seleno("ENST00000524230.9","TXNRD3",2921,2058, "no_se", 135, 2066)
```
...
write.csv(all_heatmap_df, "/SSD/maa7095/RIBOseq/Shino_new_analysis/inframe_downtream_analysis/Cell_lines/p_site_coverage/heatmap_values/NO_sel_cummulative_fraction_by_bin_prop.csv")
```
